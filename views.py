...
def readDetails(contract_type):
    ...
    deployed_contract_address = '0x4b6008698Cf7C4c2A4AEa311E962BfD3ED92674a' #hash address to access student contract
...
def saveDataBlockChain(currentData, contract_type):
    ...
    deployed_contract_address = '0x4b6008698Cf7C4c2A4AEa311E962BfD3ED92674a' #contract address
    with open(compiled_contract_path) as file:
        contract_json = json.load(file)  # load contract info as JSON
        contract_abi = contract_json['abi']  # fetch contract's abi - necessary to call its functions
    file.close()
...
def SchoolSignupAction(request):
    if request.method == 'POST':
        ...
        if status == "none":
            ...
            context = {"data":"Hospital signup task completed"}
...
def CompanySignupAction(request):
    if request.method == 'POST':
        ...
        if status == "none":
            ...
            saveDataBlockChain(data,"schoolcompany")
            context = {"data":"Organization signup task completed"}
...
def ViewStudents(request):
    if request.method == 'GET':
        ...
        arr = ['Organization Name','Donor ID','Donor Name','Organ Details','Age','Application Date']
        output += "<tr>"
...
def SendAccessRequest(request):
    if request.method == 'GET':
        ...
        output = '<table border=1 align=center width=100%>'
        font = '<font size="" color="white">'
        arr = ['Organization Name','Donor ID','Donor Name','Organ Details','Age','Application Date','Send Access Request']
        output += "<tr>"
        for i in range(len(arr)):
            output += "<th>"+font+arr[i]+"</th>"
        readDetails('enrollstudent')
...
def SendRequest(request):
    if request.method == 'GET':
        ...
        context = {"data":"Access Request sent to Donor: "+sid}
        return render(request, 'SendAccessRequest.html', context)
...
def AccessCertificate(request):
    if request.method == 'GET':
        ...
        output = '<table border=1 align=center width=100%>'
        font = '<font size="" color="white">'
        arr = ['Organization Name','Donor ID','Certificate Details','Issue Date','File Name','Hashcode','Certificate']
        output += "<tr>"
        for i in range(len(arr)):
            output += "<th>"+font+arr[i]+"</th>"
        ...
        for i in range(len(access_certificate)-1):
            ...
            if status == True:
                ...
                if os.path.exists('C:/dev/Implementation and Analysis of Blockchain Based DApp for Organ Donation/static/certificates/'+array[4]):
                    os.remove('C:/dev/Implementation and Analysis of Blockchain Based DApp for Organ Donation/static/certificates/'+array[4])
                with open('C:/dev/Implementation and Analysis of Blockchain Based DApp for Organ Donation/static/certificates/'+array[4], "wb") as file:
                    file.write(content)
                file.close()
...
def GrantAccess(request):
    if request.method == 'GET':
        ...
        output = '<table border=1 align=center width=100%>'
        font = '<font size="" color="white">'
        arr = ['Donor ID','Organization Name','Access Status','Grant Access']
        output += "<tr>"
        for i in range(len(arr)):
            output += "<th>"+font+arr[i]+"</th>"
        readDetails('accessrequest')
